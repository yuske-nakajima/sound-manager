...
| `ORIGIN_my-song_pattern-a.wav` | `pattern-a.wav` | `ORIGIN/my-song/` | 自作パターン（詳細は[ORIGIN（自作パターン）の命名規則](#originパターンの命名規則)を参照） |
...
<a id="originパターンの命名規則"></a>
### ORIGIN（自作パターン）の命名規則

`ORIGIN_` で始まるファイルは、ユーザーが SP-404 MK2 用に自作したパターン音源として特別に処理され、曲名ごとのサブディレクトリに配置されます。

```
参照元:  ORIGIN_<曲名>_<パターン名>.<拡張子>
出力先:  ORIGIN/<曲名>/<パターン名>.<拡張子>
```

| 項目 | 規則 |
| --- | --- |
| プレフィックス | `ORIGIN_`（大文字小文字を区別する完全一致。`artist_` / `tone_` とは異なり `origin_` や `Origin_` は対象外） |
| フィールド区切り | アンダースコア。ちょうど 2 個（全体で 3 フィールド） |
| 曲名 | 英数字とハイフン。アンダースコア不可。空文字不可 |
| パターン名 | 英数字とハイフン。アンダースコア不可。空文字不可 |
| 拡張子 | 対応フォーマット（`.wav` / `.mp3`）に準ずる |

単語の区切りにはハイフンを使います（例: `pattern-a`、`drum-loop-dry`）。アンダースコアはフィールド区切りとして予約されているため、曲名・パターン名には使用できません。
...
| 変換前 | 変換後 | 配置先ディレクトリ |
|--------|--------|-------------------|
| `ORIGIN_my-song_pattern-a.wav` | `pattern-a.wav` | `ORIGIN/my-song/` |
| `ORIGIN_My-Song_Pattern-A.MP3` | `Pattern-A.MP3` | `ORIGIN/My-Song/` |

#### 変換例（異常系、変換対象外）

| ファイル名 | 理由 |
|-----------|------|
| `origin_my-song_pattern-a.wav` | プレフィックスが小文字（`ORIGIN_` の大文字完全一致ではない） |
| `ORIGIN_song.wav` | フィールド不足（2 フィールドしかない） |
| `ORIGIN_my_song_pattern.wav` | フィールド過多（4 フィールド。曲名にアンダースコアは使えない） |
| `ORIGIN__pattern.wav` | 曲名が空文字 |
| `ORIGIN_song_.wav` | パターン名が空文字 |
| `ORIGIN_my#song_pattern.wav` | 記号を含む（`#` は英数字とハイフンではない） |
| `ORIGIN_a_b.txt` | 非対応拡張子（`.txt`） |

いずれも規則違反のため変換対象外となり、`export` 実行時に「no mapping found」としてスキップされます。

### 判定順序

出力先の解決は以下の順序で判定されます。アーティスト → ORIGIN → 単音 → ループ → カテゴリマッピングの順に判定し、最初に一致した規則が採用されます。

1. アーティスト判定（`artist_` プレフィックス）
2. ORIGIN 判定（`ORIGIN_` プレフィックス）
3. 単音判定（`tone_` プレフィックス）
4. ループ判定（BPM または `loop` を含むか）
5. カテゴリマッピング（`config/mapping.yaml` によるキーワード一致）

単音判定をループ判定より前に置いているのは、`tone_bass_Bb2.wav` のようなファイル名にオクターブの数字が含まれ、ループ判定が先に走ると BPM として誤解釈される恐れがあるためです。

ORIGIN 判定をループ判定より前に置いているのも同じ理由です。`ORIGIN_track-01_pattern-120.wav` のように曲名・パターン名に数字が含まれるファイル名は、ループ判定が先に走ると BPM として誤解釈されます。また `ORIGIN_my-song_drum-loop.wav` のように `loop` という語をパターン名に含む場合も、ループ判定に吸われてしまいます。
...
#### 非準拠の `tone_` / `artist_` / `ORIGIN_` ファイル

`tone_` / `artist_` / `ORIGIN_` プレフィックスが一致していても、フィールド数・文字種・BPM やオクターブの表記など規則を外れたファイル名は変換対象外です（ORIGIN の場合はアンダースコアちょうど 2 個・3 フィールドという条件を満たさないケース）。この場合、`config/mapping.yaml` によるカテゴリ変換にはフォールバックせず、`export` 実行時にそのままスキップされます。

#### 出力名の衝突

`AT/` と `CM/` と `ORIGIN/` の出力名には（他のカテゴリ出力と異なり）連番の番号サフィックスが付きません。そのため、別々のソースディレクトリに置かれた参照元でもアーティスト名・曲名・BPM（または楽器・キー・オクターブ・バリエーション、または曲名・パターン名）が同一であれば、同一の出力パスに解決されます。`--overwrite` を指定しない場合は 2 件目以降がスキップされ（`already exists`）、`--overwrite` を指定した場合は後から処理された方で上書きされます。

また、大文字小文字を区別しないファイルシステム（macOS の既定である APFS など）では、曲名・アーティスト名・楽器名の大文字小文字だけが異なるファイルも同一の出力ディレクトリに解決されます。例えば `ORIGIN_my-song_pattern-a.wav` と `ORIGIN_My-Song_pattern-b.wav` は、ファイル名変換ロジック上は別の曲名（`my-song` / `My-Song`）として扱われますが、macOS の既定のファイルシステムでは `ORIGIN/my-song/` と `ORIGIN/My-Song/` が同一ディレクトリとして扱われ、`pattern-a.wav` と `pattern-b.wav` が共存する形になります。大文字小文字を区別するファイルシステム（Linux の ext4 など）では、これらは別ディレクトリとして分離されます。

#### 小文字の `origin_` プレフィックスは ORIGIN 規則の対象外

ORIGIN のプレフィックス判定は `ORIGIN_`（大文字のみ）の完全一致に限定しています。そのため、小文字の `origin_` で始まる一般ファイル（例: `origin_120_loop.wav`）は ORIGIN 規則の対象にならず、ループ判定やカテゴリマッピングなど ORIGIN 判定を経由しない経路で変換されます。`origin_` で始まるファイルを ORIGIN 規則の対象にしたい場合は、プレフィックスを大文字の `ORIGIN_` にリネームしてください。
...
│   ├── originDetector.ts  # ORIGIN（自作パターン）検出
│   ├── originDetector.test.ts
